/*
 * Copyright (c) 2023 Space Cubics, LLC.
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include <zephyr/kernel.h>
#include "rw.h"

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(rw_test);

/*
static void print_rw_status(enum rw_pos pos, uint32_t sec)
{
	for (int j=0; j<sec*1; j++) {
		k_sleep(K_MSEC(1000));
		rw_print_cv(pos);
		LOG_INF("%s count: %d", rw_pos_name[pos], rw_get_count(pos));
	}
}
*/

static void print_rw_status_all(uint32_t sec)
{
	enum rw_pos pos_list[] = {
		RW_POS_X,
		RW_POS_Y,
		RW_POS_Z,
	};
	for (int j=0; j<sec*1; j++) {
		k_sleep(K_MSEC(1000));
		LOG_INF("--------------------------------------------------");
		for (int i=0; i<ARRAY_SIZE(pos_list); i++) {
			rw_print_cv(pos_list[i]);
		LOG_INF("%s count: %d", rw_pos_name[pos_list[i]], rw_get_count(pos_list[i]));
 		}
	}
}

int rw_test(uint32_t *err_cnt)
{
	int ret;
	int all_ret = 0;
	enum rw_pos pos_list[] = {
		RW_POS_X,
		RW_POS_Y,
		RW_POS_Z,
	};
	uint8_t pot_list[] = {
//		0x00,
		0x40,
//		0x7F,
	};

	LOG_INF("---------- Start Potention: 0x40 on X,Y,Z ---------");
	for (int i=0; i<ARRAY_SIZE(pos_list); i++) {
		ret = rw_change_speed(pos_list[i], 0x40);
		if (ret < 0) {
			(*err_cnt)++;
			all_ret = -1;
			goto end;
		}

		ret = rw_start(pos_list[i]);
		if (ret < 0) {
			(*err_cnt)++;
			all_ret = -1;
			goto end;
		}
	}

//	print_rw_status(RW_POS_X, 5);
	print_rw_status_all(5);
	
	uint8_t sec = 60;
/*	for (int i=0; i<ARRAY_SIZE(pot_list); i++) {

		LOG_INF("---------- Change Potention: 0x%02x on X ----------",
			    pot_list[i]);
		ret = rw_change_speed(RW_POS_X, pot_list[i]);
		if (ret < 0) {
			(*err_cnt)++;
			all_ret = -1;
			break;
		}

		print_rw_status(RW_POS_X, sec);
	}
*/
	for (int i=0; i<ARRAY_SIZE(pot_list); i++) {

		LOG_INF("---------- Change Potention: 0x%02x on X,Y,Z ----------",
			    pot_list[i]);

		for (int j=0; j<ARRAY_SIZE(pos_list); j++) {
			ret = rw_change_speed(pos_list[j], pot_list[i]);
			if (ret < 0) {
				(*err_cnt)++;
				all_ret = -1;
				break;
			}
		}
		print_rw_status_all(sec);
	}

	for (int i=0; i<ARRAY_SIZE(pos_list); i++) {
		rw_stop(pos_list[i]);
	}

end:
	return all_ret;
}
