...
static int csp_power_control_onoff_cmd(uint8_t command_id, csp_packet_t *packet)
{
	...
	if (onoff) {
		sc_adcs_power_enable(target);
	} else {
		sc_adcs_power_disable(target);
	}
...
static int csp_imu_enable_cmd(uint8_t command_id, csp_packet_t *packet)
{
	int ret = 0;
	uint8_t onoff;

	if (packet->length != PWRCTRL_IMU_CMD_SIZE) {
		LOG_ERR("Invalide command size: %d", packet->length);
		ret = -EINVAL;
		goto end;
	}

	onoff = packet->data[PWRCTRL_IMU_ONOFF_OFFSET];

	LOG_INF("IMU enable command (onoff: %d)", onoff);
...
int csp_pwrctrl_handler(csp_packet_t *packet)
{
	...
	case PWRCTRL_CYCLE_CMD:
		...
		break;
	case PWRCTRL_IMU_CMD:
		csp_imu_enable_cmd(command_id, packet);
		break;
	default:
